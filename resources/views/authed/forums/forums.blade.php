        ...
        <table id="tablelevel" class="mx-auto p-2 w-full md:w-[90%] whitespace-nowrap text-[0.7rem] lg:text-sm">
            ...
            <tbody>
                @foreach ($levels as $level)
                    <tr>
                        <td class="nom"><div class="flex justify-center items-center">{{ $level->sector->name }}: {{ $level->name }}</div></td>
                        <td class="actions">
                            <div class="flex justify-center items-center">
                                <a href="{{ route('forums.forum', ['level_id' => $level->id]) }}" type="submit" class="openModalView text-blue-600 text-xs p-2 border-2 border-blue-600 text-[0.7rem] lg:text-sm rounded-lg ml-3 mr-3 transition-all duration-300 ease-in-out hover:bg-blue-600 hover:text-white"><i class="fas fa-search"></i></a>
                            </div>
                        </td>
                    </tr>
                @endforeach
